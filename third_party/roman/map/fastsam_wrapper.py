...
class FastSAMWrapper():
    ...
    def __init__(self, 
        ...
        Args:
            ...
            merge_dilate_px (int, optional): Dilation radius (px) used to detect adjacency.
        """
        # parameters
    ...
    def setup_rgbd_params(
        ...
        # Preserve the historical attribute name used later in run().
        self.plane_filter_params = plane_filter_ratio
    ...
    def run(self, t, pose, img, depth_data=None):
        ...
        for mask in masks:
            ...
            if depth_data is not None:
                if self.use_pointcloud:
                    ...
                    if len(ptcld_test) < self.within_depth_frac*pre_truncate_len:
                        continue
                    depth_valid_ratio = (len(ptcld_test) / pre_truncate_len) if pre_truncate_len > 0 else 0.0
                ...
                if self.plane_filter_ratio is not None:
                    ...
                    try:
                        ...
                        if  extent[0] > self.plane_filter_ratio[0] and \
                            extent[1] > self.plane_filter_ratio[1] and \
                            extent[2] < self.plane_filter_ratio[2]:
                                continue
                    except:
                        continue
            ...
            if self.semantics == 'clip':
                ...
                dino_mask_tensor = dino_features[mask_bool] # (N, 768)
                alpha = 0.3
                beta = 0.5

                if dino_mask_tensor.shape[0] > 1:
                    variances = torch.var(dino_mask_tensor, dim=0, unbiased=True)
                    total_variance = torch.sum(variances).item()
                else:
                    total_variance = 0.0
                mask_area = float(mask_bool.sum().item())
                image_area = float(mask_bool.numel())
                area_ratio = (mask_area / image_area) if image_area > 0 else 0.0
                score = math.log1p(total_variance) * (area_ratio ** alpha) * (depth_valid_ratio ** beta)
